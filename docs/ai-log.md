# AI assistance — full log

The long form behind [`NOTES.md`](../NOTES.md): every finding with the observation that produced
it, and the prompts reproduced in full.

Kept separate because the assignment asks for a short note and this is not one. Nothing here is a
summary — if a claim in `NOTES.md` looks strong, the measurement behind it is in this file.
